...
public class MyflixDaoImpl implements MyflixDao{
	...
	public void addUsers(Users user) {
		...
		//movie.setMid(System.currentTimeMillis());
		String insertSQL = "INSERT INTO movieusers(firstname, lastname, userid, password)VALUES (?, ?, ?, ?);";
				
				jdbcTemplate.update(insertSQL,new Object[] {user.getFirstname(),user.getLastname(),user.getUserid(),user.getPassword()});
	...
	public List<Users> getAllUserIds() {
		// TODO Auto-generated method stub
		return jdbcTemplate.query("select userid from movieusers",new BeanPropertyRowMapper<Users>(Users.class));
	}

	@Override
	public boolean isRegisterdUser(Users user) {
		if(user.getUserid().equals("schada")&&user.getPassword().equals("schada")){
			return true;
		}else
		return false;
	}
